...
if(request.getParameter("email") != null){
	aszEmail=request.getParameter("email");
	if(aszEmail.length()<2){
		if(request.getParameter("proxiedEmail") != null){
			aszEmail=request.getParameter("proxiedEmail");
		}
	}
}
if(request.getParameter("nickname") != null){
	aszUsername=request.getParameter("nickname");
}
...
if(request.getParameter("firstName") != null){
	aszFirst=request.getParameter("firstName");
}
if(request.getParameter("lastName") != null){
	aszLast=request.getParameter("lastName");
}
if(request.getParameter("zip") != null){
	aszPostal=request.getParameter("zip");
}
if(request.getParameter("country") != null){
	aszCountry=request.getParameter("country");
}
if(request.getParameter("state") != null){
	aszState=request.getParameter("state");
}
if(request.getParameter("city") != null){
	aszCity=request.getParameter("city");
}	
// fix the URL encoding on the timestamp, signature, uid (make sure any spaces are spaces, - are -, : are :, etc...
aszTimestamp = aszTimestamp.replace("%20", " ").replace("%2D", "-").replace("%3A", ":").replace("%3D", "=").replace("%5F", "_").replace("%u30c3", "");
aszUID = aszUID.replace("%20", " ").replace("%2D", "-").replace("%3A", ":").replace("%3D", "=").replace("%5F", "_").replace("%u30c3", "");
aszSignature = aszSignature.replace("%20", " ").replace("%2D", "-").replace("%3A", ":").replace("%3D", "=").replace("%5F", "_").replace("%u30c3", "");
...
function createAccount() {
	...
	//document.forms.individualForm.action='<%=request.getContextPath()%>/register.do?indivaccnt';
...
<% if(	withinTimeLimit==true && ( aszUID.length()<5 || (! (aszUID.substring(0,4).equalsIgnoreCase("_gid")) ))){ %>
	document.getElementById('body').style.display='none';
	document.getElementById('processingLogin').style.display='inline';
	document.forms.individualForm.action='<%=request.getContextPath()%>/register.do';
	document.forms["individualForm"].elements["method"].value = "processLogin";
	document.forms["individualForm"].elements["personinternalcomment"].value = "gigyaSocialize";
	document.forms["individualForm"].elements["provider"].value = "<%=aszProvider%>";
	document.forms["individualForm"].elements["signature"].value = "<%=aszSignature%>";
	document.forms["individualForm"].elements["timestamp"].value = "<%=aszTimestamp%>";
	document.forms["individualForm"].elements["uid"].value = "<%=aszUID%>";
	document.forms["individualForm"].elements["email1addr"].value = document.forms["individualForm"].elements["email1addr"].value;
	document.forms["individualForm"].submit();	
<% }else{ %>		
...
<% }else{ %>
  <div id="oppsearch">
	  <span id="title">Socialize Account</span>
...
<% if(aszNoSearchNav=="true"){ %>
<div id="pagebanner">
<span style="float: left;">Socialize Account</span>
<div id="banner_side_graphic"></div>
<div id="breadcrumbs"><a href="<%=request.getContextPath()%>/register.do?method=showHome">home</a> &gt; registration</div>
</div>
<% } %>
...
<form action="<%=request.getContextPath()%>/register.do?indivaccnt" method="post" focus="username" target="_self" name="individualForm" id="individualForm">
...
<input type="image" src="<%=request.getContextPath()%>/imgs/blank.gif" /> 
</form>
...
<input type="image" name="imageField" src="<%=request.getContextPath()%>/imgs/blank.gif" width="15" height="17" title="Go" alt="Go" style="vertical-align: middle;">
</form>
