...
//console.log(('$(#srchmethod).val() is '+$('#srchmethod').val());
    ...
    Manager.store.addByValue('q', 'faith_i:21996');//ivol.org - includes secular NOT faith segmented, but not faith integrated
/*
...
//console.log(('self.manager.store.values is: ' + Manager.store.values('fq'));
	...
	if(findKey=='organization'){
		fields = [ 'primary_opp_type', 'position_type', 'service_areas', 'skills', 'great_for', 'trip_length', 'country_tax', 'region', 'province_tax', 'denom_affil', 'org_affil', 'id', 'source', 'country', 'org_member_type' ];
	}else{
		fields = [ 'position_type', 'service_areas', 'skills', 'great_for', 'frequency', 'benefits_offered', 'trip_length', 'country_tax', 'region', 'province_tax', 'denom_affil', 'org_affil', 'id', 'source', 'country', 'org_member_type' ];
	}
...
//console.log('tabswidget '+field_tabs[i]);
	...
	var params = {
	  facet: true,
	  'facet.field': [ 'service_areas', 'skills', 'great_for', 'frequency', 'benefits_offered', 'trip_length', 'country_tax', 'region', 'province_tax', 'denom_affil', 'org_affil', 'id', 'source', 'city_tax', 'position_type', 'content_type', 'org_member_type' ],
	  'facet.limit': 100,
	  'facet.mincount': 1,
...
var paramsSearch = function (response){  
	...
	Manager.addWidget(new AjaxSolr.SearchFromParamsWidget({
	  id: 'textparams',
	  target: '#params',
	  fields: [ 'service_areas', 'skills', 'great_for', 'benefits_offered', 'trip_length', 'service_areas_search', 'skills_search', 'benefits_offered_search', 'country_tax', 'province_tax', 'region', 'city_tax', 'postal_code', 'org_member_type' ]
	}));
	autocompleteSearch();
}
