...
console.log('facet_alt.value 0 postal is: ' + facet_alt.value);						
			...
			if(b_isNotANumber==true){
				...
				initialize(function(){
					...
					if(searched_geofilter==false){
						URL_hash_params.push('postal_code=' + facet_alt.value + '');
console.log('facet_alt.value postal is: ' + facet_alt.value);						
						URL_hash_params.push('fq=:{!geofilt pt=' + geo_lat + ',' + geo_long + ' sfield=latlng d=' + d_default + '}');
					}
					searched_geofilter=true;
				});
    ...
    $('#search_solr').click(function(e, facet) {
		...
		if($('#position_type_stm').attr('checked')){
			URL_hash_params.push('position_type="Short-term Missions / Volunteer Internship"');
		}
		if($('#content_type_org').attr('checked')){
			URL_hash_params.push('content_type=organization');
		}
		...
		if(query_alt.length > 0 && already_searched_alt==false 
		   && query_alt != 'Service Areas, Skills' && query != 'Keyword' && query_alt != 'Keywords (Service Areas, Skills)' && query_alt != 'Country, Postal Code' && query_alt != 'Country%2C%20Postal%20Code'){
