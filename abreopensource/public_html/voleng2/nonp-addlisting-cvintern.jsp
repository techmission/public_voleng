...
// vidBenefits=286
int iRoomBoardTID=11546, iStipendTID=11547, iMedInsurTID=11548, iTransportTID=11549, iAmeriCorpsTID=11550, iBoardMealTID=38770;
...
for(int index=0; index < aServiceList.size(); index++){
	...
	if (aszSecondaryHost.equalsIgnoreCase( "volengivol" ) && (
		...
		aszFullServiceAreaOptions += ">Disabilities Outreach</label></div>";
	}else{
		aszFullServiceAreaOptions += "<div id=\"edit-taxonomy-"+ vidService +"-"+ iSubType+"-wrapper\" class=\"form-item\" >"+
			"<label class=\"option\" for=\"edit-taxonomy-"+vidService+"-"+iSubType+"\" >" +
			"<input type=\"checkbox\" id=\"serviceareatidsarray\" name=\"serviceareatidsarray\" value=\""+ iSubType + "\" class=\"form-checkbox form-checkboxes-scroll\" ";
...
<form action="<%=aszPortal%>/org.do?method=processCreateOpportstep1" focus="opptitle" name="orgForm" enctype="multipart/form-data" method="post">
...
<% if(portal.length()>0 && showPortalInfo==true){ }else{ %>	
	...
	<div class="right-column" style="display:inline;">
		...
		<input type="radio" styleClass="radio" value="<%=aszShortTermTID%>" name="opppositiontypetid" id="opppositiontypetid" class="postypemiss" onclick="clicked_missions(addr1,city,state,postalcode,country)" <%=aszMiss%>/> Short-term <% if(! (aszSecondaryHost.equalsIgnoreCase("volengivol")) ){ %>Missions <% } %>/ Volunteer Internship<br />
...
<% if(portal.length()>0 && showPortalInfo==true && bFaith==true){ %>	
...
<!--  if b_CVCInternshipSiteApproved==true, then don't load this; it will automatically use the Internship Application form instead -->
  <div id="requireddocuments">
    ...
    <div class="right-column" id="questionnaire_section"> 
      <input type="radio" name="questionnaire_type" value="online" <%=questionnaireOnlineTypeSelected%> />
      Online
      <select id="questionnaire_id" name="questionnaire_id">
      ...
      <br />
      <input type="radio" name="questionnaire_type" value="on_paper" <%=questionnaireOnPaperTypeSelected%> />
      On Paper
      <% if(opp.getQuestionnaireServerFile() != null) { %>
        (<a href="<%=aszPortal + opp.getQuestionnairePublicPath()%>">application.<%=opp.getQuestionnaireServerFileExtension()%></a>)
      <% } %>
      <input type="file" name="questionnaire_file" />
	</div>
    <br clear="all" />
      ...
      <div id="existing_documents">
        <% if(opp.getRequiredDocumentsToShowOnForm() != null) { %>
          <% for(RequiredDocumentDTO doc : opp.getRequiredDocumentsToShowOnForm()) { %>
            <div>
              <a href="<%=aszPortal+doc.getPublicPath() %>"><%=doc.getName()%>.<%=doc.getExtension()%></a> 
...
if(aszDuplicate != null && aszDuplicate.equals("showOpportunityDuplic")){
	b_duplicate=true;
}
...
<% } %>
  ...
  function set_edit_questionnaire_link() {
    var questionnaireId = parseInt($('#questionnaire_id').val());
    var content = '';
