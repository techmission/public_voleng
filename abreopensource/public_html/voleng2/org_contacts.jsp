    ...
    <logic:iterate id="userprofile" name="userlist" type="com.abrecorp.opensource.dataobj.PersonInfoDTO">
      ...
      <TR class="<%=aszClass%>">
        ...
        <img class="email" height="17" width="22" border="0" title="Send Email to <%=userprofile.getUSPNameFirst()%>&nbsp;<%=userprofile.getUSPNameLast()%>" alt="Send Email to <%=userprofile.getUSPNameFirst()%>&nbsp;<%=userprofile.getUSPNameLast()%>" valign="middle" src="http://www.christianvolunteering.org/imgs/mail.gif"/></td>
        	...
        	<% if(b_isAdmin == true){ %>
	        	<em><%=aszOrgOrChurch%> Administrator</em>
        	<% }else{ %>
    	    	<A href="<%=aszPortal%>/org.do?method=showRemoveOrgContact&uid=<%=userprofile.getUserUID()%>&orgnid=<%=org.getORGNID()%><% if(bAdminRole==true){ %>&role=siteadmin<% } %>">Delete</A>
        	<% } %>
        </td>
...
<br><ul>
<li>All users in these lists have <em>partial</em> <b>Administrative</b> privileges on the given <%=aszOrgOrChurch%>, which allows them to add or delete Users (other Administrators) and Organizations</li>
</ul>
